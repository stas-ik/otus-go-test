...
const (
	sleepAddPerStage = time.Millisecond * 100
	addFault         = time.Millisecond * 100 // Увеличено для теста large_input
)
...
func TestAdditionalPipeline(t *testing.T) {
	...
	t.Run("simple case", func(t *testing.T) {
		...
		require.Less(t,
			int64(elapsed),
			int64(sleepAddPerStage)*int64(len(stages)+len(data)-1)+int64(addFault))
	})
	...
	t.Run("done case", func(t *testing.T) {
		...
		require.Less(t, int64(elapsed), int64(abortDur)+int64(addFault))
	})
}
...
func TestAdditionalAllStageStop(t *testing.T) {
